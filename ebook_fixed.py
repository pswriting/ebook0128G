        ...
        # 브라우저 ID가 있으면 승인 여부 확인
            ...
            if st.button("🔓 관리자로 승인", key="btn_admin_approve"):
                if admin_pw == ADMIN_PASSWORD:
                    # 1) 우선 JSONBin에 승인 시도
                    added = add_device_to_approved(device_id)
                    ...
                    if added:
                        ...
                        # 2) JSONBin 실패 → 비상 우회 (이번 세션만 통과)
                        st.session_state['authenticated'] = True
                        st.session_state['emergency_bypass'] = True
                        st.warning("⚠️ JSONBin 동기화 실패. 비상 우회로 이번 세션은 입장합니다.")
                        st.rerun()
                else:
                    st.error("관리자 비밀번호가 틀렸습니다.")
            ...
            # 🩺 진단 패널 (문제 원인 즉시 확인용)
            # ────────────────────────────────────────────
            with st.expander("🩺 진단 정보 (로그인 안 될 때 펼치세요)"):
                diag_bin_id = get_bin_id()
                st.write(f"**저장된 Bin ID:** `{diag_bin_id}`" if diag_bin_id else "**저장된 Bin ID:** ❌ 없음 (생성 실패)")
                st.write(f"**현재 기기 ID:** `{device_id}`")

                # JSONBin 직접 호출 테스트
                if diag_bin_id:
                    try:
                        test_url = f"https://api.jsonbin.io/v3/b/{diag_bin_id}/latest"
                        test_headers = {"X-Master-Key": JSONBIN_API_KEY}
                        test_resp = requests.get(test_url, headers=test_headers, timeout=10)
                        st.write(f"**JSONBin HTTP 상태:** `{test_resp.status_code}`")
                        if test_resp.status_code == 200:
                            rec = test_resp.json().get("record", {})
                            approved = rec.get("approved_devices", [])
                            st.write(f"**승인된 기기 수:** {len(approved)}")
                            st.write(f"**내 기기가 승인 목록에 있는가:** {'✅ 예' if device_id in approved else '❌ 아니오'}")
                        else:
                            st.code(test_resp.text[:300])
                    except Exception as e:
                        st.error(f"JSONBin 호출 실패: {e}")
                else:
                    st.error("Bin ID가 없습니다. 아래 '새 Bin 강제 생성' 버튼을 누르세요.")

                # 새 Bin 강제 생성 (Bin이 사라진 경우 복구용)
                if st.button("🔨 새 Bin 강제 생성", key="btn_force_bin"):
                    new_bin = create_jsonbin()
                    if new_bin:
                        st.success(f"✅ 새 Bin 생성됨: {new_bin}")
                        st.rerun()
                    else:
                        st.error("❌ Bin 생성 실패 — JSONBin API 키가 무효이거나 한도 초과")
    st.stop()
