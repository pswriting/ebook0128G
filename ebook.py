...
def create_ebook_docx(title, subtitle, author, chapters_data, outline, interview_data=None):
    ...
    try:
        ...
        prologue_text = None
        if interview_data:
            prologue_prompt = f"""당신은 한국 자기계발 베스트셀러 작가입니다. 첫 문장에서 독자를 멈춰 세우고 마지막 문장에서 다음 페이지를 넘기게 만드는 프롤로그를 작성하세요.
...
500자 안에서 독자가 "이 책 사길 잘했다"가 아니라 "지금 당장 1장으로 넘어가야 한다"고 느끼게 만들 것.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
✍️ 톤: 자기계발 후킹 + 권석천 칼럼 깊이
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

[구조 - 4단 흐름]
1) 장면 (2~3문장): 특정 시간·장소·인물의 구체적 사건. 일반론 절대 금지
   예: "2022년 가을, 새벽 두 시. 나는 같은 엑셀 파일을 47번째 열고 있었다."

2) 균열 (2~3문장): 그 장면에서 드러난 모순 또는 발견
   예: "그날 처음 깨달았다. 문제는 내가 게을러서가 아니었다."

3) 약속 (2문장): 책이 무엇을 알려주는지 한 줄로. 추상이 아니라 구체적 변화
   예: "이 책은 그 47번의 시도가 어떻게 첫 매출 30만원으로 바뀌었는지를 단계별로 풀어낸다."

4) 미끼 (1~2문장): 다음 페이지를 못 넘기게 하는 호기심 갭
   예: "그런데 그 변화의 출발점은 의외의 한 가지였다."

[문체]
- 합쇼체 기본 + 짧은 문장 위주, 가끔 긴 문장으로 호흡
- 현재형/과거형 혼용으로 장면감
- 권석천 칼럼처럼 디테일이 살아 있는 묘사 (시간, 숫자, 도구, 표정)
...
❌ 과장: "놀라운", "혁신적인", "충격적인", "어마어마한"
❌ 뻔한 말: "포기하지 마세요", "꾸준히 하세요", "당신도 할 수 있습니다"
❌ 직접 호칭: 여러분, 당신, 독자님
❌ 마크다운 문법, 글머리 기호, 이모지
...
위 시그니처 컨셉/시스템 이름은 책의 척추다. 단, 챕터마다 라벨처럼 박지 말고 PART 1과 PART 5의 마지막 소제목 두 곳에만 자연스럽게 등장시켜라.
...
각 PART당 소제목 3개. 모두 다른 패턴 + 명사·동사 엔딩 섞기.
...
특정 작가 고유어: 자청 / 역행자 / 자의식 해체 / 유전자 역행 / 원시인 / 추월차선 / 아토믹 해빗
AI 클리셰: 졸업 / 정체 / 마지막 한 수 / 다른 차원 / 결정적 시그널 / 진짜 게임 / 흔적도 없다 / 사건이 아니다
시스템 의인화: "○○가 멈춘 날", "○○ 위에 얹다", "○○를 졸업한", "회복이 굴러가다"
밍밍: 효과적인 / 성공적인 / ~의 모든 것 / ~하는 방법 / 알아야 할 / 의 중요성
유치 비유: 나침반 / 열쇠 / 보물 / 황금 / 마법 / 파이프라인 / 엔진 / 톱니바퀴 / 사이클 / 눈덩이
참고서: 첫걸음 / 완벽가이드 / 핵심정리 / 기초/중급/고급
...
- [호기심 갭형]
- [이중/삼중 보상형]
- [컨셉명 두 번째 등장: 인그룹 사회증명]

목차만 출력. 콜론 금지. 매 소제목 다른 패턴. 명사·동사 엔딩 섞기. 어법 어색한 표현 즉시 폐기."""
...
def regenerate_chapter_subtopics(chapter_name, chapter_index):
    ...
    prompt = f"""당신은 한국 자기계발 베스트셀러 편집자입니다. 이 챕터의 소제목 3개를 결제하고 싶게 써주세요.
...
- 형식: [3~4자 영문 약어 또는 한글 신조어] + 시스템/구조/공식/알고리즘/법칙 중 하나
- 좋은 예: "DPS 시스템" (Discover-Plug-Scale), "ARC 시스템" (Awareness-Reset-Control), "PAM 공식"
- ❌ 금지: "월500 시스템" 같은 직설형 / Loop·Pipeline·Engine·Module 같은 AI 클리셰

이 컨셉명을 5개 PART 중 정확히 2곳에만 자연스럽게 등장시킨다:
- PART 1의 마지막 소제목 (도입): "이 책의 모든 것은 '○○○' 세 글자에서 시작한다" 같은 자연스러운 등장
- PART 5의 마지막 소제목 (검증): "○○○를 익힌 사람들의 5년 뒤가 완전히 다른 이유" 같은 인그룹 사회증명

매 PART 챕터 제목/소제목에 컨셉명 박지 마라. 책의 척추로만 두고, 등장 자체는 두 번이면 충분.
