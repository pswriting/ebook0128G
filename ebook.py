...
.stApp {
    background:
        radial-gradient(ellipse at 50% -10%, rgba(201,162,75,0.04) 0%, transparent 55%),
        #0b0b0d !important;
...
/* 타이포그래피 - 가볍고 정제된 프리미엄 톤 */
h1, h2, h3 { color: var(--text) !important; font-weight: 300 !important; letter-spacing: 0.5px; }
h1 { font-size: 38px !important; color: var(--cream) !important; font-weight: 300 !important; }
h2 { font-size: 27px !important; margin-bottom: 20px !important; font-weight: 300 !important; }
h3 { font-size: 19px !important; color: var(--gold) !important; font-weight: 500 !important; letter-spacing: 0.3px; }
p, span, label, div { color: var(--text) !important; font-size: 16px !important; line-height: 1.75 !important; }
...
.stButton > button {
    ...
    border: 0.5px solid var(--line-gold) !important;
    border-radius: 11px;
    font-weight: 500;
...
.stButton > button[data-testid="baseButton-primary"] {
    ...
    box-shadow: 0 4px 18px rgba(201,162,75,0.18) !important;
}
.stButton > button[kind="primary"]:hover,
.stButton > button[data-testid="baseButton-primary"]:hover {
    background: linear-gradient(135deg, #ecd08a 0%, #d6ae54 100%) !important;
    color: #0b0b0d !important;
    box-shadow: 0 6px 24px rgba(201,162,75,0.3) !important;
...
.stTextInput input::placeholder, .stTextArea textarea::placeholder {
    ...
    -webkit-text-fill-color: #5c5a55 !important;
}
...
/* 섹션 타이틀 - 화려하게 */
.section-title-box {
    background: linear-gradient(135deg, rgba(201,162,75,0.15) 0%, rgba(183,110,121,0.1) 50%, rgba(201,162,75,0.15) 100%);
...
def estimate_book_pages(chapters_data, outline, prologue_text="", has_epilogue=True):
    """실제 Word 문서 구조(A5 / 소제목마다 새 페이지 / 챕터 표지 등)를 반영한 페이지 수 추정.

    create_ebook_docx의 레이아웃과 동일한 규칙으로 계산한다:
    - 표지 1p, 판권 1p, 프롤로그 N p, 목차 N p
    - 챕터마다: 챕터 표지 1p + (소제목마다 새 페이지에서 시작)
    - 에필로그 1p, 저자 소개 1p
    """
    import math
    # A5(14.8x21cm), 맑은고딕 10.5pt, 줄간격 1.85, 좌우 2.2 / 상하 2.5cm 기준 캘리브레이션
    CHARS_PER_PAGE = 760        # 본문이 꽉 찬 한 페이지에 들어가는 공백 제외 글자 수
    FIRST_PAGE_CHARS = 540      # 소제목 헤더/여백이 차지하는 첫 페이지의 본문 용량

    def _clen(s):
        return len((s or "").replace(" ", "").replace("\n", ""))

    pages = 2  # 표지 + 판권

    # 프롤로그
    pages += max(1, math.ceil(_clen(prologue_text) / CHARS_PER_PAGE)) if prologue_text else 1

    # 목차 (챕터 줄 + 소제목 줄, 페이지당 약 28줄)
    toc_lines = len(outline) + sum(
        len(chapters_data.get(ch, {}).get('subtopics', [])) for ch in outline
    )
    pages += max(1, math.ceil(toc_lines / 28))

    # 본문
    for ch in outline:
        cd = chapters_data.get(ch, {})
        subs = cd.get('subtopics', [])
        sd = cd.get('subtopic_data', {})
        if not any(sd.get(s, {}).get('content', '') for s in subs):
            continue
        pages += 1  # 챕터 표지 페이지
        for s in subs:
            content = sd.get(s, {}).get('content', '')
            if not content:
                continue
            try:
                n = _clen(clean_content(content, subtopic=s))
            except Exception:
                n = _clen(content)
            if n <= FIRST_PAGE_CHARS:
                pages += 1
            else:
                pages += 1 + math.ceil((n - FIRST_PAGE_CHARS) / CHARS_PER_PAGE)
...
def count_book_pages(docx_bytes, chapters_data, outline, prologue_text="", has_epilogue=True):
    """가능하면 LibreOffice로 docx→pdf 변환해 실제 페이지를 정확히 세고,
    실패하면 구조 기반 추정값을 반환한다.

    반환값: (페이지수:int, 방식:str)  방식 = "exact" | "estimate"
    """
    if docx_bytes:
        try:
            ...
            if soffice:
                ...
                with tempfile.TemporaryDirectory() as tmp:
                    dp = os.path.join(tmp, "book.docx")
                    with open(dp, "wb") as f:
                        f.write(docx_bytes)
                    subprocess.run(
                        [soffice, "--headless", "--convert-to", "pdf", "--outdir", tmp, dp],
            ...
            pass
    return estimate_book_pages(chapters_data, outline, prologue_text, has_epilogue), "estimate"
...
if header_video_b64:
    ...
    @keyframes writeyGlow {{
        0%, 100% {{ text-shadow: 0 0 20px rgba(201,162,75,0.5), 0 0 40px rgba(201,162,75,0.3), 0 0 60px rgba(201,162,75,0.2); }}
        50% {{ text-shadow: 0 0 30px rgba(201,162,75,0.8), 0 0 60px rgba(201,162,75,0.5), 0 0 90px rgba(201,162,75,0.3); }}
    }}
    ...
    .writey-title {{
        ...
        font-style: italic;
        background: linear-gradient(135deg, #fff 0%, #c9a24b 25%, #fff 50%, #c9a24b 75%, #fff 100%);
    ...
    .writey-subtitle {{
        ...
        color: rgba(201,162,75,0.9) !important;
        font-size: 15px !important;
    ...
    <div style="position:relative;border-radius:24px;overflow:hidden;margin-bottom:35px;box-shadow:0 15px 50px rgba(0,0,0,0.5), 0 0 100px rgba(201,162,75,0.1);">
    ...
    @keyframes writeyGlowFallback {{
        0%, 100% {{ text-shadow: 0 0 20px rgba(201,162,75,0.5); }}
        50% {{ text-shadow: 0 0 40px rgba(201,162,75,0.8); }}
    }}
    </style>
    <div style="text-align:center;padding:60px 20px;margin-bottom:30px;background:linear-gradient(180deg, rgba(20,20,20,0.9) 0%, rgba(10,10,10,0.95) 100%);border-radius:20px;border:1px solid rgba(201,162,75,0.2);">
        <div style="color:rgba(201,162,75,0.9);font-size:14px;letter-spacing:6px;margin-bottom:15px;font-weight:500;">✦ CASHMAKER ✦</div>
        <h1 style="font-family:'Playfair Display',serif;font-size:72px;font-weight:700;font-style:italic;color:#c9a24b;margin:0;letter-spacing:6px;animation:writeyGlowFallback 3s ease-in-out infinite;">Writey</h1>
...
if not st.session_state.get('interview_completed', False):
    ...
    if step == 1:
        st.markdown("""
        <div style="background:linear-gradient(135deg, rgba(201,162,75,0.1) 0%, rgba(183,110,121,0.05) 100%);padding:30px;border-radius:15px;border-left:4px solid var(--gold);margin-bottom:30px;">
...
if not get_api_key():
    ...
    <div style="background:linear-gradient(135deg, rgba(201,162,75,0.2) 0%, rgba(30,30,30,0.98) 100%);
                border:3px solid rgba(201,162,75,0.6);border-radius:25px;padding:50px 40px;text-align:center;margin:20px 0;">
    ...
    # 마무리
    st.markdown("""
    <div style="background:linear-gradient(135deg, #c9a24b 0%, #b8860b 100%);padding:30px;border-radius:20px;margin:40px 0;text-align:center;">
...
if current == 0:
    ...
    with col1:
        ...
        # 빠른 제작 버튼 (자동 모드)
        st.markdown("""
        <div style="background:linear-gradient(135deg, #c9a24b 0%, #b8860b 100%);padding:20px;border-radius:15px;margin:20px 0;text-align:center;">
...
# ==========================================
    ...
    if not st.session_state.get('outline'):
        ...
        if full_content:
            ...
            _pages = estimate_book_pages(
                st.session_state.get('chapters', {}),
                st.session_state.get('outline', []),
            )
            st.success(f"총 {char_count:,}자 | 약 {_pages}페이지")
...
{escaped_content}
        ...
        if total > 0:
            _pages, _mode = count_book_pages(
                docx_data if DOCX_AVAILABLE else None,
                st.session_state.get('chapters', {}),
                st.session_state.get('outline', []),
            )
            if _mode == "exact":
                st.success(f"총 {total:,}자 | {_pages}페이지 (실제 문서 기준)")
            else:
                st.success(f"총 {total:,}자 | 약 {_pages}페이지")
