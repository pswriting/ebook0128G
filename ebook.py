...
[작명 원칙 - 반드시 '이 주제에서만 나올 수 있는 이름'으로]
• 주제의 고유한 한국어 단어 1~2개에서 파생시킨 한글 조어 (예: 배당 투자라면 배당 관련 단어, 블로그라면 글/검색 관련 단어)
• 어미는 시스템/공식/구조/법칙/루틴 중 하나
• 영문 약어(DIP, SLP, CPM, 3R 같은 3~4글자 알파벳)는 금지 — 어느 주제에나 붙는 이름은 컨셉이 아니다
• 다른 책에서 본 듯한 이름이면 폐기하고 다시 지어라
