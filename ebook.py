...
{{
    ...
    "subtitle": "부제 (구체적 결과/약속, 15~25자)"
        ...
        outline_prompt = f"""당신은 자청입니다. 목차만으로 독자를 홀려야 합니다.
목차를 보는 순간 "이거 안 사면 손해다"라는 생각이 들게 만드세요.
...
"PART X. [도발적인 한 문장]" (콜론 금지!)

✅ 자청식 좋은 예 (이런 느낌으로):
- "PART 1. 왜 똑같이 해도 나만 안 되는가"
- "PART 2. 아무도 안 알려주는 진짜 원리"
- "PART 3. 이것만 알면 경쟁이 무의미해진다"
- "PART 4. 하루 30분으로 결과를 만드는 구조"
- "PART 5. 월 100만 원이 월 1000만 원이 되는 과정"
...
소제목을 보면 "오 이건 꼭 읽어야겠다" 싶어야 함

[도발형] "~는 완전히 틀렸다", "~하면 망한다"
[비밀형] "상위 1%만 쓰는 방법", "절대 공개 안 하는 ~"
[실전형] "당장 써먹는 3단계", "오늘부터 바로 적용하는 ~"
[반전형] "노력이 문제가 아니다", "열심히 할수록 멀어지는 이유"
[결과형] "이것 하나로 ~가 달라졌다", "2주 만에 ~한 방법"
    ...
    try:
        ...
        for ch in st.session_state['outline']:
            ...
            for sub in ch_data['subtopics']:
                ...
                content_prompt = f"""당신은 자청입니다. 몰입감 있는 글을 씁니다.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
❌ 유치한 표현: 후다닥, 짜잔, 대박, ㅋㅋ
❌ 유치한 비유: 마법, 황금열쇠, 나침반, 로켓
❌ 억지 메타포: 순환법, 엔진, 고리, 파이프라인, 톱니바퀴
❌ AI스러운: 중요합니다, 따라서, 결론적으로
❌ 형식: 1. 2. 첫째, 둘째, 글머리 기호
❌ HTML 태그나 표 사용 금지
...
def generate_content_premium(subtopic, chapter, questions, answers, topic, persona):
    ...
    prompt = f"""당신은 자청입니다. '{subtopic}'에 대해 몰입감 있게 씁니다.
...
- 숫자를 써라: "많이" X → "월 340만원" O
- 사례를 써라: "수강생 민준씨", "친구 지수"
- 실제 도구명: 네이버, 구글, 엑셀 등
...
- 시작: {current_hook}
- 몰입감 있게, 자청처럼
- {'비교표 1개 포함' if include_table else ''}"""
    return ask_ai(prompt, 0.75)
