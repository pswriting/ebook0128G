...
def build_cover_svg(template, title, subtitle, author):
    """베스트셀러 10대 아키타입 표지 SVG (1600x2560, 무손실 벡터)"""
    ...
    def wrap(maxc):
        lines = [html.escape(l) for l in _wrap_title_lines(title, max_chars=maxc, max_lines=3)]
        longest = max((len(l) for l in lines), default=4)
        return lines, longest

    def title_block(x, y_center, lines, fs, family, weight, fill, anchor="middle", ls=3, lh=1.22):
        line_h = int(fs * lh)
        top = y_center - int((len(lines) - 1) * line_h / 2)
        spans = "".join(f'<tspan x="{x}" dy="{0 if i == 0 else line_h}">{l}</tspan>' for i, l in enumerate(lines))
    ...
    def fit(longest, base, mid, small):
        return base if longest <= 4 else (mid if longest <= 6 else small)

    if template == "editorial":
        lines, lg = wrap(7); fs = fit(lg, 220, 180, 145)
        t, top, lh = title_block(800, 1170, lines, fs, serif, 600, '#1A1A1A')
        body = (
            '<rect width="1600" height="2560" fill="#F4F0E8"/>'
            '<rect x="78" y="78" width="1444" height="2404" fill="none" stroke="#1A1A1A" stroke-width="2"/>'
            '<rect x="100" y="100" width="1400" height="2360" fill="none" stroke="#8A7A55" stroke-width="0.8" opacity="0.6"/>'
            f'<line x1="640" y1="{top - 150}" x2="960" y2="{top - 150}" stroke="#8A7A55" stroke-width="3"/>{t}'
            + (f'<text x="800" y="1640" text-anchor="middle" font-family="{serif}" font-size="44" fill="#5A5346" letter-spacing="6">{sub}</text>' if sub else "")
            + (f'<text x="800" y="2330" text-anchor="middle" font-family="{serif}" font-size="40" fill="#8A7A55" letter-spacing="10">{auth}</text>' if auth else "")
        )
    elif template == "deep_navy":
        lines, lg = wrap(7); fs = fit(lg, 220, 180, 145)
        t, top, lh = title_block(800, 1170, lines, fs, serif, 600, '#F3ECD9')
        body = (
            '<defs><radialGradient id="g1" cx="50%" cy="38%" r="72%"><stop offset="0%" stop-color="#1B3457"/><stop offset="100%" stop-color="#0B1626"/></radialGradient></defs>'
            '<rect width="1600" height="2560" fill="url(#g1)"/>'
            '<rect x="70" y="70" width="1460" height="2420" fill="none" stroke="#C9A24B" stroke-width="2" opacity="0.7"/>'
            f'<line x1="660" y1="{top - 130}" x2="940" y2="{top - 130}" stroke="#C9A24B" stroke-width="3"/>'
            '<rect x="780" y="1560" width="40" height="40" transform="rotate(45 800 1580)" fill="none" stroke="#C9A24B" stroke-width="2"/>'
            + t
            + (f'<text x="800" y="1700" text-anchor="middle" font-family="{serif}" font-size="44" fill="#C9C3B2" letter-spacing="6">{sub}</text>' if sub else "")
            + (f'<text x="800" y="2350" text-anchor="middle" font-family="{serif}" font-size="40" fill="#C9A24B" letter-spacing="10">{auth}</text>' if auth else "")
        )
    elif template == "minimal_type":
        lines, lg = wrap(6); fs = fit(lg, 200, 165, 135)
        t, top, lh = title_block(150, 760, lines, fs, serif, 600, '#F2EDE3', anchor="start", ls=2)
        body = (
            '<rect width="1600" height="2560" fill="#0F3D2E"/>'
            f'<line x1="150" y1="{top - 160}" x2="420" y2="{top - 160}" stroke="#C8B27C" stroke-width="4"/>{t}'
            + (f'<text x="150" y="{top + len(lines)*lh + 60}" text-anchor="start" font-family="{serif}" font-size="42" fill="#A9C0B4" letter-spacing="3">{sub}</text>' if sub else "")
            + '<line x1="150" y1="2280" x2="1450" y2="2280" stroke="#C8B27C" stroke-width="1" opacity="0.5"/>'
            + (f'<text x="150" y="2380" text-anchor="start" font-family="{serif}" font-size="38" fill="#C8B27C" letter-spacing="8">{auth}</text>' if auth else "")
        )
    elif template == "color_block":
        lines, lg = wrap(6); fs = fit(lg, 230, 190, 150)
        t, top, lh = title_block(150, 980, lines, fs, sans, 900, '#FFFFFF', anchor="start", ls=0, lh=1.18)
        body = (
            '<rect width="1600" height="2560" fill="#1D4ED8"/>'
            '<rect x="0" y="2120" width="1600" height="440" fill="#0B0F1E"/>'
            f'<rect x="150" y="{top - 220}" width="120" height="26" fill="#FFD43B"/>{t}'
            + (f'<text x="150" y="2280" text-anchor="start" font-family="{sans}" font-size="46" font-weight="500" fill="#FFFFFF" letter-spacing="2" opacity="0.95">{sub}</text>' if sub else "")
            + (f'<text x="150" y="2400" text-anchor="start" font-family="{sans}" font-size="38" font-weight="700" fill="#FFD43B" letter-spacing="6">{auth}</text>' if auth else "")
        )
    elif template == "big_number":
        m = re.search(r'\d[\d,\.]*', f"{title} {subtitle or ''}")
        bignum = html.escape(m.group(0)) if m else html.escape((title or "1")[:1])
        nfs = 1050 if len(bignum) <= 2 else (760 if len(bignum) <= 3 else 540)
        lines, lg = wrap(7); fs = fit(lg, 190, 158, 128)
        t, top, lh = title_block(800, 1560, lines, fs, sans, 900, '#111111', ls=0, lh=1.2)
        body = (
            '<rect width="1600" height="2560" fill="#F5F2EC"/>'
            f'<text x="800" y="1080" text-anchor="middle" font-family="{sans}" font-size="{nfs}" font-weight="900" fill="none" stroke="#D8483A" stroke-width="6" opacity="0.9">{bignum}</text>'
            + t
            + (f'<text x="800" y="{top + len(lines)*lh + 50}" text-anchor="middle" font-family="{sans}" font-size="44" font-weight="500" fill="#555555" letter-spacing="2">{sub}</text>' if sub else "")
            + (f'<text x="800" y="2380" text-anchor="middle" font-family="{sans}" font-size="38" font-weight="700" fill="#D8483A" letter-spacing="6">{auth}</text>' if auth else "")
        )
    elif template == "gradient_modern":
        lines, lg = wrap(6); fs = fit(lg, 210, 175, 140)
        t, top, lh = title_block(800, 1130, lines, fs, sans, 800, '#FFFFFF', ls=1, lh=1.2)
        body = (
            '<defs><linearGradient id="g2" x1="0%" y1="0%" x2="100%" y2="100%">'
            '<stop offset="0%" stop-color="#4338CA"/><stop offset="55%" stop-color="#7C3AED"/><stop offset="100%" stop-color="#0D9488"/></linearGradient></defs>'
            '<rect width="1600" height="2560" fill="url(#g2)"/>'
            '<circle cx="1280" cy="420" r="300" fill="none" stroke="#FFFFFF" stroke-width="1.5" opacity="0.35"/>'
            '<circle cx="320" cy="2160" r="420" fill="none" stroke="#FFFFFF" stroke-width="1" opacity="0.25"/>'
            f'<line x1="660" y1="{top - 140}" x2="940" y2="{top - 140}" stroke="#FFFFFF" stroke-width="3" opacity="0.9"/>{t}'
            + (f'<text x="800" y="1680" text-anchor="middle" font-family="{sans}" font-size="44" font-weight="400" fill="#E8E6FF" letter-spacing="3">{sub}</text>' if sub else "")
            + (f'<text x="800" y="2360" text-anchor="middle" font-family="{sans}" font-size="38" font-weight="700" fill="#FFFFFF" letter-spacing="8" opacity="0.9">{auth}</text>' if auth else "")
        )
    elif template == "warm_essay":
        lines, lg = wrap(7); fs = fit(lg, 185, 152, 125)
        t, top, lh = title_block(800, 1230, lines, fs, serif, 500, '#5B4636', ls=2)
        body = (
            '<rect width="1600" height="2560" fill="#F7EFE6"/>'
            '<circle cx="800" cy="640" r="270" fill="#F2C9A8" opacity="0.85"/>'
            '<circle cx="980" cy="540" r="90" fill="#E8A87C" opacity="0.6"/>'
            + t
            + (f'<text x="800" y="{top + len(lines)*lh + 60}" text-anchor="middle" font-family="{serif}" font-size="42" fill="#9A8470" letter-spacing="4">{sub}</text>' if sub else "")
            + '<line x1="700" y1="2230" x2="900" y2="2230" stroke="#C8A287" stroke-width="2"/>'
            + (f'<text x="800" y="2330" text-anchor="middle" font-family="{serif}" font-size="38" fill="#8A7361" letter-spacing="8">{auth}</text>' if auth else "")
        )
    elif template == "geometric":
        lines, lg = wrap(6); fs = fit(lg, 200, 168, 135)
        t, top, lh = title_block(150, 880, lines, fs, sans, 900, '#111111', anchor="start", ls=0, lh=1.18)
        body = (
            '<rect width="1600" height="2560" fill="#FAFAF7"/>'
            '<polygon points="1600,0 1600,760 840,0" fill="#111111"/>'
            '<rect x="1380" y="620" width="140" height="140" fill="#E03E2F"/>'
            f'<rect x="150" y="{top - 200}" width="220" height="18" fill="#E03E2F"/>{t}'
            + (f'<text x="150" y="{top + len(lines)*lh + 70}" text-anchor="start" font-family="{sans}" font-size="46" font-weight="500" fill="#444444" letter-spacing="1">{sub}</text>' if sub else "")
            + '<line x1="150" y1="2300" x2="1450" y2="2300" stroke="#111111" stroke-width="3"/>'
            + (f'<text x="150" y="2400" text-anchor="start" font-family="{sans}" font-size="38" font-weight="700" fill="#111111" letter-spacing="5">{auth}</text>' if auth else "")
        )
    elif template == "red_punch":
        lines, lg = wrap(6); fs = fit(lg, 225, 185, 148)
        t, top, lh = title_block(800, 1090, lines, fs, sans, 900, '#E8362B', ls=0, lh=1.16)
        body = (
            '<rect width="1600" height="2560" fill="#0C0C0C"/>'
            + t +
            f'<line x1="520" y1="{top + (len(lines)-1)*lh + 90}" x2="1080" y2="{top + (len(lines)-1)*lh + 90}" stroke="#E8362B" stroke-width="10"/>'
            + (f'<text x="800" y="{top + (len(lines)-1)*lh + 210}" text-anchor="middle" font-family="{sans}" font-size="46" font-weight="500" fill="#F2F2F2" letter-spacing="3">{sub}</text>' if sub else "")
            + (f'<text x="800" y="2370" text-anchor="middle" font-family="{sans}" font-size="38" font-weight="700" fill="#E8362B" letter-spacing="8">{auth}</text>' if auth else "")
        )
    else:  # noir_gold
        lines, lg = wrap(7); fs = fit(lg, 220, 180, 145)
        t, top, lh = title_block(800, 1170, lines, fs, serif, 700, '#E9D9A8')
        body = (
            '<defs><radialGradient id="g3" cx="50%" cy="40%" r="75%"><stop offset="0%" stop-color="#1C1A14"/><stop offset="60%" stop-color="#0E0D0B"/><stop offset="100%" stop-color="#060606"/></radialGradient></defs>'
            '<rect width="1600" height="2560" fill="url(#g3)"/>'
            '<rect x="64" y="64" width="1472" height="2432" fill="none" stroke="#C9A24B" stroke-width="1.5" opacity="0.55"/>'
            '<rect x="84" y="84" width="1432" height="2392" fill="none" stroke="#C9A24B" stroke-width="0.75" opacity="0.35"/>'
            f'<line x1="640" y1="{top - 130}" x2="960" y2="{top - 130}" stroke="#C9A24B" stroke-width="2"/>{t}'
            + (f'<text x="800" y="1680" text-anchor="middle" font-family="{serif}" font-size="44" fill="#B8B3A8" letter-spacing="6">{sub}</text>' if sub else "")
            + (f'<text x="800" y="2360" text-anchor="middle" font-family="{serif}" font-size="40" fill="#C9A24B" letter-spacing="10">{auth}</text>' if auth else "")
        )
