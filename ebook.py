...
- 주어-서술어 호응: "전략이 돌아간다", "공식이 깨어난다", "구조가 숨쉰다" 같은 추상명사 의인화 절대 금지. 동작의 주어는 사람·돈·통장·알림 같은 실물로 ("나는 D부터 시작했다", "통장에 두 번째 알림이 울렸다").
...
- 약어 해부: 컨셉이 영문 약어라면(예: DTS), 컨셉을 본격 소개하는 챕터(PART 3~4)의 소제목들에서 각 글자가 대략 무엇을 가리키는지 드러나게 하라. 독자가 목차만 보고 "D는 ~쯤, T는 ~쯤, S는 ~쯤이구나" 그림을 그릴 수 있어야 산다.
  예: "D, 월급보다 먼저 움직이는 돈" / "T를 누르는 순간 일어나는 일" / "S가 18개월 뒤 만들어낸 숫자"
