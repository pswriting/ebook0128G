...
- PART 2~4의 마지막 소제목은 "다음 PART를 안 읽고는 못 배기는 떡밥"이어야 한다.
  ✅ 미해결 + 결과 예고형: "그런데 진짜 문제는 따로 있었다", "여기서 끝이었다면 이 책을 쓰지 않았다"
  ✅ 결정적 발견 직전형: "모든 걸 뒤집은 발견은 가장 사소한 곳에 있었다"
  ⚠️ 단, 세 챕터가 같은 패턴을 반복하면 안 됨 — 각각 다른 방식의 예고로
